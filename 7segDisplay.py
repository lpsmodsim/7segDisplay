...
def emulate7SegDisplay(digit):
    ...
    if digit & (1 << 1):
        sevenSegDisplay.alpha_composite(
            segmentVertical,   (165, 35))  # top right side
    if digit & (1 << 2):
        sevenSegDisplay.alpha_composite(
            segmentVertical,   (165, 195))  # bottom right side
    if digit & (1 << 3):
        sevenSegDisplay.alpha_composite(segmentHorizontal, (35, 325))  # bottom
    if digit & (1 << 4):
        sevenSegDisplay.alpha_composite(
            segmentVertical,   (5, 195))   # bottom left side
    if digit & (1 << 5):
        sevenSegDisplay.alpha_composite(
            segmentVertical,   (5, 35))    # top left side
    if digit & (1 << 6):
...
displays[0].save('output.gif', save_all=True,
                 append_images=displays[1:], optimize=True,
                  loop=0, duration=2000)
